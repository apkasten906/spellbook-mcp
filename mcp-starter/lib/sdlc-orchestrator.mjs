...
export function planSdlc({ scope = 'feature', goal = '', phases = null, cwd = process.cwd() } = {}) {
  ...
  const gitSha = getGitShortSha(cwd);
    ...
    switch (p) {
      case 'requirements':
        return {
          ...
          artifact: `docs/PDCA/${goalSlug}-${p}-${gitSha || isoNow()}.md`,
        };
      case 'analysis':
        return {
          phase: 'analysis',
          ...
          artifact: `docs/API/${goalSlug}-${p}-${gitSha || isoNow()}.md`,
        };
      case 'architecture':
        return {
          phase: 'architecture',
          ...
          artifact: `docs/ADR/${goalSlug}-${p}-${gitSha || isoNow()}.md`,
        };
      case 'implementation':
        return {
      ...
      case 'testing':
        return {
          ...
          artifact: `docs/TESTS/${goalSlug}-${p}-${gitSha || isoNow()}.md`,
        };
      case 'deployment':
        return {
          phase: 'deployment',
          ...
          artifact: `docs/CI/${goalSlug}-${p}-${gitSha || isoNow()}.yaml`,
        };
      case 'maintenance':
        return {
          phase: 'maintenance',
          ...
          artifact: `docs/RCA/${goalSlug}-${p}-${gitSha || isoNow()}.md`,
        };
      default:
