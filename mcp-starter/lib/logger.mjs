...
export const info = (...parts) => logger.info(parts.map(p => (typeof p === 'string' ? p : JSON.stringify(p))).join(' '));
export const debug = (...parts) => logger.debug(parts.map(p => (typeof p === 'string' ? p : JSON.stringify(p))).join(' '));
export const warn = (...parts) => logger.warn(parts.map(p => (typeof p === 'string' ? p : JSON.stringify(p))).join(' '));
export const error = (...parts) => logger.error(parts.map(p => (typeof p === 'string' ? p : JSON.stringify(p))).join(' '));
